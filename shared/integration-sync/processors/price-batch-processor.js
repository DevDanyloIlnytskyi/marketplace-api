const { upsertPrice, isPriceDomainError } = require('../../catalog/price-write');
const { mapChunkRecordToPriceInput } = require('./price-record-mapper');

/**
 * Process one price batch — batch transaction + per-record savepoints.
 *
 * @param {ReturnType<import('../../tenant/model-registry').defineTenantModels>} models
 * @param {import('sequelize').Sequelize} sequelize
 * @param {import('sequelize').Model} batch
 */
async function processPriceBatch(models, sequelize, batch) {
  ...
  try {
    for (const record of records) {
      ...
      try {
        const input = mapChunkRecordToPriceInput(record);
        const result = await upsertPrice(models, input, { transaction: savepoint });
