/**
 * Run Platform-5.3 migration for integration_idempotency_keys on a single tenant database.
 *
 * Usage:
 *   node scripts/migrate-integration-idempotency.js up
 *   node scripts/migrate-integration-idempotency.js down
 */
require('dotenv').config();

const { Sequelize } = require('sequelize');
const migration = require('../migrations/20260615200000-create-integration-idempotency-keys');
...
async function main() {
  if (!['up', 'down'].includes(DIRECTION)) {
    console.error('Usage: node scripts/migrate-integration-idempotency.js [up|down]');
    process.exit(1);
  }
